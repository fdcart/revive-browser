...
class LiveSessionStore {
  ...
  async frame(sessionId: string, quality?: number): Promise<Buffer> {
    const session = this.get(sessionId);
    const frameQuality = Math.max(25, Math.min(90, quality || session.frameQuality));
    return session.page.screenshot({ type: 'jpeg', quality: frameQuality, fullPage: false });
  }

  setFrameQuality(sessionId: string, quality: number): void {
    const session = this.get(sessionId);
    session.frameQuality = Math.max(25, Math.min(90, quality));
  }

  getSessionInfo(sessionId: string): { id: string; createdAt: number; lastActiveAt: number; frameQuality: number } {
    const session = this.get(sessionId);
    return {
      id: session.id,
      createdAt: session.createdAt,
      lastActiveAt: session.lastActiveAt,
      frameQuality: session.frameQuality
    };
  }
